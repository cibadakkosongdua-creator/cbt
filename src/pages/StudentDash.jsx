            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-3">
                <div className="hidden text-right sm:block">
                  ...
                  <div className="mt-1 text-[10px] font-bold text-indigo-600 uppercase tracking-wider">
                    Kelas {user?.kelas || "–"}
                  </div>
                </div>
                <div className={`flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-white shadow-md transition-transform hover:scale-105 active:scale-95 ${ALL_ITEMS.borders.find(b => b.id === (user?.activeBorder || "none"))?.class || "ring-2 ring-slate-100"}`}>
                  {user?.photoURL ? (
                    <img src={user.photoURL} alt="Avatar" className="h-full w-full object-cover" />
                  ) : (
                    <div className={`flex h-full w-full items-center justify-center bg-gradient-to-br text-sm font-bold text-white ${getAvatarBg(user?.name)}`}>
