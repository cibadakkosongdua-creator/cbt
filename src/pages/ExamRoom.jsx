...
const ExamRoom = ({
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      // 2. Admin Message
      if (data.admin_message && data.admin_message.sentAt > (window._lastMsgAt || 0)) {
        window._lastMsgAt = data.admin_message.sentAt;
        Swal.fire({
          title: "Pesan dari Admin",
          text: data.admin_message.text,
          icon: "info",
  ...
  const handleFinishExam = async (silent = false) => {
    ...
    try {
      ...
      const passScore = (examConfig.minScore !== undefined && examConfig.minScore !== null) 
        ? Number(examConfig.minScore) 
        : 70;

      const isLulus = scoreData.score >= passScore;
      const examResultId = preview ? "preview" : `${user.id}_${Date.now()}`;

      // ── Calculate XP & Level Up ──
      let earnedXp = XP_REWARDS.SUBMIT_EXAM;
      if (scoreData.score >= 100) earnedXp += XP_REWARDS.PERFECT_SCORE;
      else if (scoreData.score >= 90) earnedXp += XP_REWARDS.KILAT_KUNING;
      if (isLulus) earnedXp += XP_REWARDS.PASS_KKM;

      const currentTotalXp = (user.xp || 0) + earnedXp;
      const { level: newLevel } = calculateLevel(currentTotalXp);
      
      // Tentukan exam_type secara cerdas berdasarkan mapel jika tidak diset di config
      const detectedExamType = (mapel === "literasi" || mapel === "numerasi") ? "TKA" : (examConfig.examType || "MAPEL");
      ...
      if (!preview) {
        const { error: insertError } = await supabase.from("results").insert({
          ...
          student_name: user.name,
          kelas: user.kelas,
          exam_name: examConfig.title || "Ujian",
          ...
          duration_limit: Number(examConfig.duration || 60),
        ...
        if (insertError) {
          console.error("[ExamRoom] Failed to save result:", insertError);
        ...
        updateUser({ xp: currentTotalXp, level: newLevel });
      }

      // Selebrasi Confetti
      if (scoreData.score >= 100 || isLevelUp) {
        confetti({
          particleCount: 150,
          spread: 70,
          origin: { y: 0.6 },
      ...
      Swal.fire({
        ...
        html: `<div class="text-center py-2">
          ...
          <p class="text-5xl font-black ${isLulus ? "text-emerald-600" : "text-orange-500"} mb-3">${scoreData.score}</p>
          ${isLevelUp ? `<p class="mb-2 text-indigo-600 font-black animate-bounce">Selamat! Kamu naik ke Level ${newLevel}</p>` : ""}
