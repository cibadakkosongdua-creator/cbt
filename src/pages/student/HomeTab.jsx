  ...
  return (
    ...
    >
      <div className="relative">
        <div className="h-12 w-12 overflow-hidden rounded-full bg-white border-2 border-white shadow-sm ring-2 ring-indigo-100">
          {user?.photoURL ? (
            <img src={user.photoURL} alt="User" className="h-full w-full object-cover" />
          ) : (
            <div className={`flex h-full w-full items-center justify-center bg-gradient-to-br ${getAvatarGrad(user?.name)} text-white text-xs font-black`}>
      ...
      <div className="min-w-0 flex-1">
        <div className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-0.5">Siswa Aktif:</div>
...
const StatCard = ({ icon, iconBg, iconColor, label, value, prefix, className = "", delay = 0 }) => (
  <motion.div 
    ...
    className={`card p-5 border-transparent hover:border-indigo-100 transition-colors ${className}`}
  >
...
const HomeTab = ({
  ...
  const isLocked = useMemo(() => {
    if (!examConfig?.isActive) return true;
    if (countdown?.type === "start") return true;
    const now = Date.now();
        ...
        <div className="relative overflow-hidden rounded-3xl animate-fade-in-up delay-100 shadow-xl shadow-slate-200">
          ...
          <div className="relative z-10 p-6 md:p-7">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4">
              ...
              {/* COUNTDOWN TIMER */}
              {countdown && (
                <div className={`flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-black tracking-wider uppercase ${
                  ...
                  <span>
                    {countdown.type === "start" ? "Mulai Dalam: " : "Berakhir Dalam: "}
                    {(() => {
                      const t = formatCountdown(countdown.diff);
                      const parts = [];
                      if (t.days > 0) parts.push(`${t.days}h`);
                      if (t.hours > 0 || t.days > 0) parts.push(`${String(t.hours).padStart(2, '0')}j`);
                      parts.push(`${String(t.mins).padStart(2, '0')}m`);
                      parts.push(`${String(t.secs).padStart(2, '0')}s`);
                      return parts.join(" ");
                    })()}
                  </span>
                </div>
                ...
                {examConfig?.examType === "TKA" ? (
                  <>
                    <button
                      ...
                      disabled={configLoading || isLocked}
                    ...
                    >
                      ...
                      {!isLocked && !configLoading && <i className="fas fa-arrow-right text-xs ml-1" />}
                    </button>
                    <button
