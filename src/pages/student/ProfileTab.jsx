...
import { computeStudentStats, ALL_ITEMS, BADGES, calculateLevel } from "../../lib/achievements";
...
const ProfileTab = ({ user, history = [], examConfig = {} }) => {
  ...
  return (
    <div className="space-y-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                <div className="flex h-6 px-2 items-center justify-center rounded-lg bg-white/20 text-[10px] font-black text-white backdrop-blur-md ring-1 ring-white/30" title="Level Kamu">
                  LV.{levelData.level}
                </div>
                {hasKilatKuning && (
      ...
      {/* ══════ EXPERIENCE & PROGRESS ══════ */}
      <div className="card overflow-hidden bg-white p-5 shadow-sm border border-slate-100">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs font-bold text-slate-400 uppercase tracking-widest">Progres Level</div>
              <div className="text-lg font-black text-slate-900">Level {levelData.level}</div>
            </div>
          </div>
          <div className="text-right">
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Total Pengalaman</div>
            <div className="text-sm font-black text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full inline-block">
              {user?.xp || 0} XP
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-[11px] font-bold">
            <span className="text-slate-500">Level {levelData.level}</span>
            <span className="text-indigo-600">{Math.floor(levelData.remainingXp)} / {levelData.xpNeeded} XP ke Level {levelData.level + 1}</span>
          </div>
          <div className="relative h-4 w-full overflow-hidden rounded-full bg-slate-100 p-1 shadow-inner">
            <div
              className="h-full rounded-full bg-gradient-to-r from-indigo-500 via-violet-500 to-fuchsia-500 transition-all duration-1000 ease-out shadow-[0_0_12px_rgba(99,102,241,0.5)]"
          ...
          <p className="text-[10px] text-slate-400 text-center italic mt-2">
            "Teruslah belajar dan kerjakan ujian untuk mendapatkan lebih banyak XP!"
          </p>
        </div>
      </div>
