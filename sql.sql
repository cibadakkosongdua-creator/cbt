...
-- ────────────────────────────────────────────────────────────
-- 12. RPC FUNCTIONS (Atomic Submission)
-- ────────────────────────────────────────────────────────────

-- Fungsi untuk submit ujian secara aman di sisi server
...
BEGIN
  ...
  FOR v_q IN SELECT * FROM questions WHERE LOWER(subtes) = LOWER(p_mapel) LOOP
    ...
    IF v_user_ans IS NOT NULL THEN
      -- PG Scoring
      IF v_q.type = 'PG' THEN
        IF (v_user_ans->>0)::text = (v_q.answer->>0)::text THEN
          v_correct := v_correct + 1;
        END IF;
      -- PGK Scoring (All or Nothing)
  ...
  -- 7. Cleanup Session
  DELETE FROM sessions WHERE id = p_student_id;
...
-- ────────────────────────────────────────────────────────────
-- 13. FIX RELATIONS (Run this if you have existing tables)
-- ────────────────────────────────────────────────────────────
